...
const DeploymentModuleFreshV3 = buildModule("DeploymentModuleFreshV3", (m) => {
  ...
  // 1. Deploy the new custom router from its new location
  const simpleSwapRouter = m.contract("SimpleSwapRouter", [], {
    id: "SimpleSwapRouter",
  });

  // 2. Deploy Entrypoint and pass the custom router's address
  const entrypoint = m.contract(
    "Entrypoint",
    [ZERO_ADDRESS, simpleSwapRouter], // Pass router address
    {
      libraries: {
